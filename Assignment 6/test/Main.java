...
public class Main {
 ...
 static JFrame frameGameOver; //Pop up frame when the game ends
 ...
 public static KeyListener GameAction = new KeyListener() {  
 ...
 public static void makeMove(int col) {
  ...
  if (previousLetter==Board.C) {
   board.makeMove(col, Board.P);
  } else {
   board.makeMove(col, Board.C);}
 ...
 int MoveY = 75 * row;
  ...
  ChipPiece.setBounds(27 + MoveX, 27 + MoveY, 68,68); //Shifts where the chip icon will be placed depending on the row and column
